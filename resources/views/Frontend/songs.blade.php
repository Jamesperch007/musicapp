                ...
                <div class="release">
                    <div class="release__content">
                        <div class="release__cover">
                            <img src="{{ asset('Frontend/img/covers/NEPMusic.jpg') }}" alt="">
                        </div>
                    ...
                    <div class="release__list">
                        ...
                        <ul class="main__list main__list--playlist main__list--dashbox">
                            <li class="single-item">
                                <a data-playlist data-title="{{ $value->song_name }}" data-artist="{{ $value->artist_name }}" data-img="{{ asset('upload/Song/' . $value->image) }}" href="{{ asset('upload/Song/' . $value->audio) }}" class="single-item__cover">
                                    <img src="{{ asset('upload/Song/' . $value->image) }}" alt="">
                                ...
                                <div class="single-item__title">
                                    <h4>{{ $value->song_name }}</h4>
                                    <span>{{ $value->artist_name }}</span>
                                </div>
                                <a href="#" class="single-item__add favorite" data-song-id="{{ $value->id }}">
                                ...
                                <a href="#" class="single-item__add add-to-playlist" data-song-id="{{ $value->id }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
...
<script>
    $(document).ready(function() {
        $('.add-to-playlist').on('click', function(e) {
            e.preventDefault();
            var songId = $(this).data('song-id');
            var playlistId = $(this).prev('.playlist-select').val(); // Get the selected playlist ID
            ...
            $.ajax({
                url: `/playlists/${playlistId}/add-song`,
                method: 'POST',
                data: {
                    song_id: songId,
                    _token: "{{ csrf_token() }}"
...
<style>
    .playlist-select {
        ...
        -webkit-appearance: none; /* For older versions of Safari and Chrome */
